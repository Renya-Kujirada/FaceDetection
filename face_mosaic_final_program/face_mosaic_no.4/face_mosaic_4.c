...
#define AREARATIO 0.0005 /* 顔の画像全体の面積に対する下限(0.5%) */
#define AREARATE  0.3   /* 外接四角形における占める割合の下限(30%) */
#define HWMIN     0.65   /* 縦横比の下限(0.8) */
#define HWMAX     2.75   /* 縦横比の上限 (1.8)*/
#define SYMMAX    0.57   /* 左右対称度の上限(0.3) */
#define NOP	  40		/*検出最大人数(10)*/
...
/**********処理１．肌色画素の検出**********/
/*＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊肌色画素の検出＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊*/
void skin_color_detection(RGB_PACKED_IMAGE *input,RGB_PACKED_IMAGE *output){
	int j,k;				//for文カウンター変数
	...
	double Y=0.0,Cb=0.0,Cr=0.0;				//Y…輝度,Cb,Cr…色差
		for(k=0;k<input->rows;k++){
			...
			//◀◀条件：(色相<50 または210<色相) かつ (-50<色差<-20) かつ (20<色差<40) かつ (輝度>100)▶▶
				if(H!=-1000){
					if((H<48||H>202)&&(-60<Cb&&Cb<-20)&&(20<Cr&&Cr<40)){
                	...
                	if ( ( H/W > HWMIN ) && ( H/W < HWMAX )){
                    		...
                    		if ( colrange > 120 ){
                        		...
                        		if ( symratio < SYMMAX ){
                        	    		...
                        	    		return n;	// 1) ~ 5)の条件を全て満たす場合、ラベル番号を顔領域としてラベル番号を返す（領域の選別）
                   	    		}
        	 		}
                	}	
            	}
...
/**********処理５．領域の左上端,右下端の座標を用いて、顔の周りを緑の外接四角形で囲む**********/
/*＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊四角形を書く関数＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊*/
...
/**********処理６．領域の左上端,右下端の座標を用いて、顔の大きさから目の部位を計算し、モザイク加工**********/
/*＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊モザイク型の四角形を書く関数＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊*/
...
#endif
{
	...
	char *name_img_in = "image4.ppm" ;  			/* 入力画像ファイル名 */
	char *name_img_out_1 = "output1_Flesh_color_area.ppm" ;  	/* 肌色領域の画像ファイル名 */
	char *name_img_out_2 = "output2_face_area.ppm" ;  		/* 顔画像ファイル名 */
	char *name_img_out_3 = "output3_result_detect.ppm" ;  	/* 顔抽出結果画像ファイル名 */
	char *name_img_out_4 = "output4_result_mosaic.ppm" ;  	/* モザイク処理結果画像ファイル名 */
	...
	/*処理開始*/	
	skin_color_detection(image_in,image_Skin_P);		//処理１．入力画像から肌色画素を抽出する

	int number = labeling(image_Skin_P,255,255,255,label);		//処理２．肌色領域の数を調べ、numeberに代入
	...
	for(a=1;a<=number;a++){							//顔候補の領域のラベル番号を調べる。
		temp=cal_area_label(image_Skin_P,label,a);			//処理３．図形特徴の算出及び顔の領域の選別
		if(temp!=0){
			if(cnt==NOP){}
			else{
	...
	for(a=0;a<NOP;a++){
		if(face_label_num[a]!=0){
			face(image_Skin_P,image_face,label,face_label_num[a]);	//処理４．処理３の結果から、顔領域のみを抽出し、書き込む
		}
	}
	printf("\n＊＊＊\t\t\t顔候補の領域を出力しました\t\t＊＊＊\n＊＊＊\t\t\tファイル名：%s\t\t＊＊＊\n\n",name_img_out_2);	
										//メッセージ出力
	...
	for(a=0;a<NOP;a++){							//顔候補の領域の端を調べる。
		if(face_label_num[a]!=0){
			SOP++;
			edge(image_Skin_P,label,face_label_num[a],UL,LR);	//領域の左上端,右下端を判定
			draw_rect(image_in,UL[0],UL[1],LR[0],LR[1],4);		//処理５．顔の周りを緑の外接四角形で囲む
			draw_rect_mosaic(image_in_1,UL[0],UL[1],LR[0],LR[1],4);	//処理６．顔の大きさから目の部位を計算し、モザイク加工
		}
	}
	printf("\n＊＊＊\t\t\t%d人分の顔の判定を判定できました\t\t＊＊＊\n",SOP);
